...
describe("fixture ai provider", () => {
  it("returns the fixture registered for the file key", async () => {
    ...
    const result = await ai.extractInvoice({ fileKey: "uploads/abc.pdf", promptVersion: 1, promptBody: "x", mode: "text" });
  ...
  it("validates the fixture through the same Zod schema as the real provider (A7)", async () => {
    ...
    await expect(ai.extractInvoice({ fileKey: "uploads/bad.pdf", promptVersion: 1, promptBody: "x", mode: "text" })).rejects.toThrow();
  });

  it("reports zero cost, so ai_calls stays honest", async () => {
    const ai = createFixtureAiProvider({ "uploads/abc.pdf": canonical });
    const { usage } = await ai.extractInvoice({ fileKey: "uploads/abc.pdf", promptVersion: 1, promptBody: "x", mode: "text" });
  ...
  it("fails loudly for an unknown file key instead of inventing an invoice (A8)", async () => {
    ...
    await expect(ai.extractInvoice({ fileKey: "uploads/missing.pdf", promptVersion: 1, promptBody: "x", mode: "text" }))
  ...
  it("treats a fixture explicitly registered as undefined as invalid input, not as an unregistered key", async () => {
    ...
    let thrown: unknown;
    try {
      await ai.extractInvoice({ fileKey: "uploads/explicit-undefined.pdf", promptVersion: 1, promptBody: "x", mode: "text" });
    } catch (err) {
      thrown = err;
    }
