...
describe("end-to-end evaluation against runRules", () => {
  function activeRuleFor(slug: string, shadow: boolean): ActiveRule {
    const rule = DETERMINISTIC_RULES.find((r) => r.slug === slug)!;
    return {
      slug: rule.slug,
      version: 1,
      spec: rule.spec,
  ...
  // Task 4 (E2) landed: `runRules` no longer throws the RF-120 boundary stub
  // for a non-empty rule set (see packages/core/src/rules/engine.ts). RN-008
  // is the one rule among these eleven whose spec needs nothing beyond
  // `pattern`'s plain regex matching — no `formula`/`expr` string — so it is
  // the one this test can exercise for real today.
  it("fires RN-008 for real on its firing fixture, and not on its clean one", () => {
    const activeRule = activeRuleFor(RN_008, false);
    const { fires, clean } = DETERMINISTIC_FIXTURES[RN_008]!;

    const firing = runRules({
      invoice: fires.invoice, previous: fires.previous, rules: [activeRule],
      answers: {}, references: { tariffs: [], flags: [] },
    });
    expect(firing).toHaveLength(1);
    expect(firing[0]?.ruleSlug).toBe(RN_008);

    const clear = runRules({
      invoice: clean.invoice, previous: clean.previous, rules: [activeRule],
      answers: {}, references: { tariffs: [], flags: [] },
    });
    expect(clear).toEqual([]);
  });

  // The other ten rules' `formula`/`expr` strings still use deterministic.ts's
  // own "provisional expression notation" (see that file's module doc
  // comment) — `item()`, `items()`, `exists()`, `count()`, ternary `?:` —
  // none of which `packages/core/src/rules/evaluators/expression.ts`'s real,
  // closed grammar implements (see that module's own doc comment for what it
  // deliberately can and cannot express). Reconciling the two notations is
  // out of Task 4's scope (engine.ts and its test only); until it happens,
  // running one of these ten through `runRules` throws `ExpressionError`
  // from `evaluateExpression`, not the old E2-not-implemented stub this test
  // used to assert. RN-001's formula (`items('multa').sumCents`) fails at
  // the earliest possible point, tokenizing: `expression.ts` only has a
  // grammar for double-quoted string literals, so the single quote right
  // after `items(` is not a character its tokenizer recognises at all —
  // the error is not even "unknown function", it never gets that far.
  it("still cannot run RN-001 end to end: its formula uses notation expression.ts's real grammar does not implement", () => {
    const activeRule = activeRuleFor(RN_001, true);
    const { invoice, previous } = DETERMINISTIC_FIXTURES[RN_001]!.fires;

    expect(() =>
      runRules({ invoice, previous, rules: [activeRule], answers: {}, references: { tariffs: [], flags: [] } }),
