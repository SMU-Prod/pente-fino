...
describe("adminOverview", () => {
  /** The SEO corpus `seedAll` leaves behind, counted the way adminOverview counts it. */
  async function seoPagesByStatus(): Promise<Record<string, number>> {
    const rows = await ctx.db.select({ status: seoPages.status }).from(seoPages);
    const counts: Record<string, number> = {};
    for (const row of rows) counts[row.status] = (counts[row.status] ?? 0) + 1;
    return counts;
  }

  // "Empty" means empty of *this* product's activity - no invoices, no
  // cases, no findings. It does not mean an empty database: `createTestDb`
  // runs `seedAll`, and since E10 that seeds the published SEO corpus. So
  // the SEO counter is asserted against the seeded baseline rather than
  // against zero, and the baseline is read rather than written down - a
  // literal here would break again the next time the corpus grows, which is
  // exactly how this test failed when E10 and E11 met.
  it("returns zeroes on a database with no activity rather than throwing", async () => {
    const seeded = await seoPagesByStatus();
  ...
  it("counts invoices since local (America/Sao_Paulo) midnight, AI cost, stalled cases, shadow findings and seo pages by status", async () => {
    const seededSeo = await seoPagesByStatus();
    ...
    // The three rows this test inserted, on top of whatever the seeds left.
    expect(overview.seoPages).toEqual({
      ...seededSeo,
      draft: (seededSeo.draft ?? 0) + 1,
      published: (seededSeo.published ?? 0) + 2,
    });
  });
});
