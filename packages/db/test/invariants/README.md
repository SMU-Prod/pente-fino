...
| INV-010 | `suppressors.spec.ts` | `packages/db` | Duas camadas. (1) Checagem de nome: regras `active` **e** `shadow` (RF-125/RF-126 — uma regra em `shadow` já grava achados reais e pode ser promovida sem revisão humana), slug comparado de forma normalizada (separador e sufixo de versão ignorados), não por igualdade exata de string. (2) Propriedade mais forte: roda o motor (`runRules`) sobre uma fatura fixture para cada uma das três teses mortas, com os três supressores de `RN_090`/`RN_091`/`RN_092` (`packages/db/src/seeds/rules/suppressors.ts`) semeados como `active`, mais uma regra "desonesta" com slug que não menciona a tese — prova que a supressão não depende do nome da regra que sinalizou. Essa segunda camada ainda não roda de verdade: `runRules` continua lançando o stub do RF-120 (Task 4 em paralelo) para qualquer conjunto de regras não vazio, então o teste documenta esse estado pendente explicitamente em vez de fingir passar — fica ativo sozinho assim que o motor deixar de lançar |
